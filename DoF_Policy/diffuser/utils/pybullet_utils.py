...
def get_image(
    ...
    **kwargs
    ...
    focal_length = get_focal_lengths(height, vertical_fov)  
...
#     p.resetJointState(body, joint, targetVelocity=velocity, physicsClientId=CLIENT) 
...
def get_closest_angle_fn(
    ...
    def closest_fn(q1, q2):
        ...
        angle_and_distance = []
        for direction in [0, PI] if reversible else [0]:  
...
def get_nonholonomic_distance_fn(body, joints, **kwargs):
    ...
    # distance_fn = get_distance_fn(body, joints, **kwargs) 
...
def waypoint_joint_controller(
    ...
    dt = get_time_step()  
    time_elapsed = 0.0
    # print(duration_fn(get_joint_positions(body, joints), target)) 
...
def joint_controller_hold2(
    ...
    while not np.allclose(current_conf, target_positions, atol=tolerance, rtol=0):
        ...
        p.setJointMotorControlArray(
            ...
            # maxVelocities=[0.]*len(movable_joints), 
            # forces=forces,
            physicsClientId=CLIENT,
        )
        yield current_conf
    ...
    **kwargs
    ...
    for attempt in irange(max_attempts):
        if (len(solutions) >= max_solutions) or (elapsed_time(start_time) >= max_time):
            break
        if not first_close or (attempt >= 1):  
            sub_conf = sample_fn()
